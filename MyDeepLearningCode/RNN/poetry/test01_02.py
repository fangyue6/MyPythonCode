# -*- coding: utf-8 -*-
"""
Created on Fri Jun 16 00:44:12 2017
...
def predict_by_words(model,words,seq_len):
    words_text = words.lower()
     
    char_to_int = dict((c, i) for i, c in enumerate(chars))  
    int_to_char = dict((i, c) for i, c in enumerate(chars))
    
    n_chars = len(words_text)  
    n_vocab = len(chars)  
...
def load_data1(filename, seq_len, normalise_window):
...
def build_model1(layers):  #layers [1,50,100,1]
    model = Sequential()
...
if __name__=='__main__':
    ...
    X_train, y_train, n_vocab = load_data(seq_len)
    print('X_train shape:',X_train.shape)  #(3709L, 50L, 1L)
    print('y_train shape:',y_train.shape)  #(3709L,)
    
    print('> Data Loaded. Compiling...')

    model = build_model([1, 64, 100, n_vocab])
    model.fit(X_train,y_train,batch_size=512,nb_epoch=epochs,validation_split=0.05)
    predict_by_words(model,'When Dad was younger When Dad was younger When Dad was younger',seq_len)
